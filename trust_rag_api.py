...
MAX_SNIPPETS = int(os.getenv("MAX_SNIPPETS", "20"))         # allow many snippets
MAX_CONTEXT_CHARS = int(os.getenv("MAX_CONTEXT_CHARS", "24000"))  # big context for comprehensive answers
UPLOAD_MAX_BYTES = 12 * 1024 * 1024  # 12MB cap per file

# -------------------- APP --------------------
app = FastAPI(title="Private Trust Fiduciary Advisor API")

app.add_for_router_middleware = True
app.add_middleware(
    CORSMiddleware,
...
RATE_WINDOW = 10
RATE_LIMIT = 100  # more permissive for comprehensive answers
...
def _clean_title(title: str) -> str:
    t = title or "Unknown"
    t = re.sub(r'^[Ll]\d[_\-:\s]+', '', t)             # drop leading L1_ etc.
    t = re.sub(r'(?i)\bocr\b', '', t)                  # strip OCR tag
    t = re.sub(r'[0-9a-f]{8,}', '', t, flags=re.I)     # drop long hashes
    if ' -- ' in t:
        first, *rest = t.split(' -- ')
        if len(first) >= 6:
            t = first
...
def _citation_line(s: dict) -> str:
    """Title (doc_id, L#, p.#, v.#) — only include fields that exist; titles only (no filenames)."""
    meta = s.get("meta", {}) or {}
    parts = []
    if meta.get("doc_id"):
...
# -------------------- Synthesis (mirror your GPT policy; no brevity limits) --------------------
def synthesize_answer(question: str, unique_sources: list[dict], snippets: list[str]) -> str:
    ...
    # Build a large context from many snippets (bounded by MAX_CONTEXT_CHARS)
    context, used = "", 0
    ...
    # Your exact policy block (verbatim spirit), embedded to match GPT voice & structure
    policy_block = (
        ...
        "Core Functionality\n"
        "• Dynamic Intake & Validation… • Instrument Profiling & Generation… • Administration & Oversight… • Research & Exegesis… • Strategic Structuring…\n\n"
        "Outputs: Prescriptive memoranda with embedded citations; exportable ledgers/resolutions/indices; audit-proof summaries with layered authorities.\n\n"
        "Citation Formatting Rules: Statutes '26 U.S.C. § 641' (or 'IRC § 641'); Regulations 'Treas. Reg. § 1.641(a)-0'; Cases (e.g., Gregory v. Helvering, 293 U.S. 465 (1935)); Revenue Rulings (e.g., Rev. R. 79-47, 1979-1 C.B. 312); Restatement (Third) of Trusts § 78 (2007); Treatises (e.g., Scott & Ascher on Trusts § 17.2 (5th ed. 2007)). "
        ...
        "You are a retrieval-augmented legal research assistant. Your only source of information is the connected Trust-Law RAG context provided below. "
        ...
        "Cite in-line with formal legal citation where the snippet permits, and conclude with a single block labeled 'Citations' listing Title (doc_id, L#, p.#, v.#) for the retrieved authorities."
    )

    system_msg = (
        "You are the 'Private Trust Fiduciary Advisor'. Produce a comprehensive, formal legal analysis. "
        "Do not shorten for brevity. Use only the provided context."
    )
    user_msg = (
        f"{policy_block}\n\n"
        f"QUESTION:\n{question}\n\n"
        f"CONTEXT (verbatim snippets from authoritative sources):\n{context}\n\n"
        f"TASK: Provide a comprehensive memorandum-style answer in legal register, organized by the layered method "
        ...
        f"Ensure every substantive proposition is grounded in the context and carries an appropriate parenthetical or footnote-style citation if the snippet allows. "
        f"Conclude with exactly one block titled 'Citations' listing the deduplicated authorities below, then append the one-line Disclaimer verbatim.\n\n"
        f"Citations (Title (doc_id, L#, p.#, v.#)):\n{citations}"
    )
    ...
    try:
        chat = client.chat.completions.create(
            model=SYNTH_MODEL,
            temperature=0.15,
            max_tokens=1800,   # allow long, comprehensive output
            messages=[{"role":"system","content":system_msg},
                      {"role":"user","content":user_msg}],
        )
        text = (chat.choices[0].message.content or "").strip()
        return text if text else "No relevant material found in the Trust-Law knowledge base."
    except Exception as e:
        return f"(Synthesis unavailable: {e})"

# -------------------- Widget (black text, no separate sources box) --------------------
...
<style>
  ...
  .card{background:#fff;border:1px solid #e5e5ea;border-radius:12px;padding:14px;margin-top:12px;color:#000;white-space:pre-wrap}
  .muted{color:#444}
</style>
</head>
<body>
...
async function askRag(q){
  const u=new URL('/rag',location.origin);u.searchParams.set('question',q);u.searchParams.set('top_k','10');
...
F.addEventListener('submit',async(e)=>{
  e.preventDefault(); OUT.innerHTML='<div class="card">Working…</div>';
  try{
    let data;
    ...
    OUT.innerHTML='<div class="card">'+ans+'</div>';
  }catch(err){ OUT.innerHTML='<div class="card">Error: '+String(err)+'</div>'; }
});
</script>
</body>
</html>"""
...
def rag_endpoint(
    ...
    try:
        ...
        results = idx.query(vector=emb, top_k=max(top_k, 10), include_metadata=True, filter=flt)
