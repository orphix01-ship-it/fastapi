...
def synthesize_html(question: str, uniq_sources: List[Dict[str, Any]], snippets: List[str]) -> str:
    ...
    try:
        ...
        critique_user_msg = (
            ...
            "or computed value compared to a threshold, verify the conclusion "
            "direction matches the math. For §7520 PV factor calculations "
            "specifically: PV factor = 1/(1+r)^n, so as the rate INCREASES, "
            "the PV factor DECREASES. This means: at LOWER §7520 rates, the "
            "PV factor is HIGHER and MORE likely to exceed the 5% threshold "
            "(triggering grantor trust status under §673); at HIGHER §7520 "
            "rates, the PV factor is LOWER and LESS likely to exceed the 5% "
            "threshold (NOT triggering). If the analysis states the opposite "
            "direction (e.g., \"triggers when rate is high\", or \"does not "
            "trigger at 5% rate but would at lower rate\"), it is "
            "mathematically inverted and MUST be corrected. The actuarial "
            "tool compute_7520_pv_factor is available to verify any specific "
            "claim. Quick check: at 5% rate, 25-year reversion PV factor is "
            "29.5%; at 12.73% breakeven rate, PV factor is exactly 5%; above "
            "12.73%, PV factor is below 5% (no trigger).\n\n"
            "9. PREMISE-CONCLUSION CHECK (single-sentence consistency): "
            "Any sentence that states a premise (e.g., \"the PV factor "
            ...
            "must support the stated conclusion, not contradict it.\n\n"            "10. TERM-LENGTH NEUTRALITY (post-TRA86): IRC § 673(a) is "
            "purely a 5%-of-value test; there is NO term-length component "
            "in current law. Do NOT assume that a longer term (30 years, "
            "40 years, etc.) makes the trust LESS likely to trigger § 673 "
            "— the OPPOSITE may be true depending on the § 7520 rate. Longer "
            "term reversions at low rates trigger MORE easily, not less, "
            "because the discount is gentler. Reasoning like \"a long-term "
            "reversion is too remote to count\" is pre-TRA86 thinking and "
            "is NOT current law. Always defer to the computed PV factor "
            "vs. the 5% threshold; the term length alone tells you "
            "nothing.\n\n"
